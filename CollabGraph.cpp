 ...
 **       refactored map from string->Artist to string->Vertex*
     12 Dec 2021: Nathan Solomon 
     multiple functions implemented to class
 **/
...
void CollabGraph::populate_graph(const vector<Artist>& artists) {
    ...
    //populating graph with vertices
    for (int i = 0; i < size; i++) {
        insert_vertex(artists[i]);
    }
    //compare each artist with every artist in the list for a potential 
    //song collabaration. If there is one, create an edge between the two 
    //artists
    for (int i = 0; i < size; i++) {
        for (int z = i + 1; z < size; z++) {
            collabedSong = artists[i].get_collaboration(artists[z]);
            // skip inserting songs that are empty strings
            if (collabedSong != "") {
                insert_edge(artists[i], artists[z], collabedSong);
            }
        }
    }
}
...
vector<Artist> CollabGraph::get_vertex_neighbors(const Artist& artist) const {
    ...
    //find the artist's vertex that we are looking for in the graph
    Vertex* currVertex = graph.at(artist.get_name());
    // copy the vertex's neighbors into a vertex
    vector <Edge> edgesCopy = currVertex->neighbors;
    int numEdges = edgesCopy.size();
    //save each artist that is a neighbor to the original vertex to a list 
    // of neighbors and return the vector
    for (int i = 0; i < numEdges;i++) {
        Artist neighborArtist = edgesCopy.at(i).neighbor->artist;
        neighbors.push_back(neighborArtist);
    }
    return neighbors;
}
...
stack<Artist> CollabGraph::report_path(const Artist& source,
    ...
    stack<Artist> path;
    // getting the pointer to the vertex where the destination Artist is
    Vertex* currVertex = graph.at(dest.get_name());
    // pushing destination artist onto stack
    path.push(currVertex->artist);
    //if there is no path from the destination to the source when backtracking,
    // then there is no path and is marked by returning an empty stack
    if (currVertex->predecessor == nullptr) {
        stack<Artist> emptyStack;
        return emptyStack;
    }
    //continuously backtrack and save the predecessor path until reaching 
    // the source
    while(currVertex->artist != source and currVertex->predecessor != nullptr) {
